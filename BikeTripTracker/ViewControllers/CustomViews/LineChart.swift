...
// MARK: - Got it from https://github.com/zemirco/swift-linechart
...
open class LineChart: UIView {
    ...
    */
    ...
    public struct Grid {
		...
		public var color: UIColor = UIColor.placeholderText//UIColor(red: 238/255.0, green: 238/255.0, blue: 238/255.0, alpha: 1)
    }
...
//        }
		...
		// My code
		for index in 0..<Int(x.grid.count) {
			let labelWidth = CGFloat(Int(frame.width) / x.labels.values.count)
			let xValue = CGFloat(index) * labelWidth
			let label = UILabel(frame: CGRect(x: xValue, y: y, width: labelWidth, height: x.axis.inset))
			label.font = UIFont.preferredFont(forTextStyle: UIFont.TextStyle.caption2)
			label.textAlignment = .center
			if x.labels.values.indices.contains(index) {
				text = x.labels.values[index]
			} else {
				text = String(index)
			}
